...
export function verifyPassword(plain: string, stored: string): boolean {
	...
	const m = /^pbkdf2\+sha512\$(\d+)\$([0-9A-Za-z+/=]+)\$([0-9A-Za-z+/=]+)$/.exec(stored);
	if (m) {
...
/** Stored value equals the hex SHA-256 digest of the password. */
export function isSha256Digest(plain: string, stored: string): boolean {
