...
export function mapWorkloadDetail(e: WorkloadEntry, services: KubeService[]): ServiceDetail {
	...
	const bindMounts: ServiceDetail["bindMounts"] = [];
	const volumeMounts: ServiceDetail["volumeMounts"] = [];
	...
	for (const v of volumes) {
		...
		if (v.hostPath?.path) {
			...
			volumeMounts.push({
				containerPath: mount?.path ?? "",
				volumeName: v.persistentVolumeClaim.claimName,
	...
	const publishedPorts: ServiceDetail["publishedPorts"] = [];
	const podLabels = template?.metadata?.labels ?? {};
	for (const s of services) {
		if (s.metadata?.namespace !== ns) continue;
		if (!selectorMatches(s.spec?.selector ?? {}, podLabels)) continue;
		for (const p of s.spec?.ports ?? []) {
			publishedPorts.push({
				containerPort: typeof p.targetPort === "number" ? p.targetPort : (p.port ?? 0),
				hostPort: p.nodePort ?? null,
				protocol: (p.protocol ?? "TCP").toUpperCase(),
				mode: (s.spec?.type ?? "ClusterIP").toLowerCase(),
			});
		}
	}
	...
	return {
		...
		labels: Object.entries(meta.labels ?? {}).map(([key, value]) => ({ key, value })),
		publishedPorts,
		bindMounts,
		volumeMounts,
...
/** Parse a Kubernetes quantity (`10Gi`, `500m`, `1e9`) to bytes-ish number. */
export function parseQuantityBytes(q: string | undefined): number | undefined {
...
function mapStamped(s: KubeStamped): StampedSummary {
...
export function validateManifestYaml(yamlText: string): Array<Record<string, unknown>> {
	...
	try {
		...
		throw new ManifestValidationError(e instanceof Error ? e.message : String(e));
	}
...
export class KubernetesOrchestrator implements Orchestrator {
	...
	async listConfigs(): Promise<StampedSummary[]> {
		...
		return configMaps.map(mapStamped);
	}
	...
	async clusterDisplayName(): Promise<string | null> {
		if (this.cfg.instanceName) return this.cfg.instanceName;
		return this.kube.contextName() ?? "kubernetes";
	}
