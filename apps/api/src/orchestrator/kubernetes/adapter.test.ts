...
describe("parseQuantity", () => {
	it("parses binary and decimal suffixes", () => {
		...
		expect(parseQuantity("10Gi")).toBe(10 * 1024 ** 3);
		expect(parseQuantity("5G")).toBe(5e9);
		expect(parseQuantity("123")).toBe(123);
		expect(parseQuantity("wat")).toBeUndefined();
	});
});
...
describe("KubernetesOrchestrator over the mock apiserver", () => {
	...
	it("exposes service detail with env and published ports", async () => {
		...
		expect(detail!.env).toEqual(["LOG_LEVEL=info"]);
		expect(detail!.ports.some((p) => p.includes("30080"))).toBe(true);
	});
	...
	it("reports healthy cluster", async () => {
		const o = orch();
